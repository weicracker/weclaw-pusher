	...
	for {
		...
		if err != nil {
			...
			if m.failures == maxConsecutiveFailures {
				log.Printf("[monitor] WARNING: %d consecutive failures. If this persists, run `weclaw-pusher login` to re-authenticate.", maxConsecutiveFailures)
			}
			select {
		...
		if resp.ErrCode == errCodeSessionExpired {
			if m.getUpdatesBuf != "" {
				...
				log.Printf("[monitor] WARNING: WeChat session expired and cannot be auto-recovered. Run `weclaw-pusher login` to re-authenticate.")
			}
			select {
